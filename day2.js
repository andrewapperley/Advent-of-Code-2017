var SpreadSheet = (function () {
    ...
    SpreadSheet.part1 = function () {
        var total = 0;
        var formatted = SpreadSheet.input.split('\n').map(function (row) {
            return row.split('\t').map(Number);
        });
        formatted.forEach(function (row) {
            row.sort(function (l, r) {
                if (l < r) {
                    return -1;
                }
                else if (l > r) {
